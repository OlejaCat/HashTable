...
Выделим методы по итогу тестирования, которыми будем пользоваться. Лучше всего себя показали хеш-фукнции: полиномиальный, crc32, sdbm. Для исследований была выбрана **crc32**, так как можно увидеть меньшее количество длинных цепочек и из-за дальнейшей возможности векторизовать вычисления.
...
В итоге получили ускорение для построения `17%` и `8%` для построения + поиска относительно `-O2`. И на `43%` и на `22%` относительно без флагов.
