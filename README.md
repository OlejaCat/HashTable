...
**Интринсики** — это специальные обёртки над ассемблерными инструкциями типа SIMD.
Преимущества:
...
Тесты запускались без припрепления программы к определённому ядру. Для стабилизации частоты и температуры использовались разогревочные запуски программы.
...
![Optimizstion_3](screenshots/optimization_3.png)
...
| Без флагов               | `740.3 мс ± 7.7 мс`      | `2.872 с ±  0.034 с`     |
| -O2                      | `605.0 мс ± 4.7 мс`      | `2.546 с ±  0.044 с`     |
| Свой memcmp              | `576.8 мс ± 4.7 мс`      | `2.515 c ±  0.034 с`     |
| Inline memcmp + SIMD     | `576.8 мс ± 4.7 мс`      | `2.447 c ±  0.010 c`     |
| Crc32 SSE4 + быстрый DIV | `515.0 мс ± 3.8 мс`      | `2.352 с ±  0.008 с`     |

И теперь таблицу с ускорением относительно компиляции с флагом -O2

| **Флаги компиляции**     | **Построение таблицы** | **Построение + поиск**   |
|--------------------------|------------------------|--------------------------|
| -O2                      | 1.00x                  | 1.00x                    |
| Свой memcmp              | 1.05x                  | 1.01x                    |
| Inline memcmp + SIMD     | 1.05x                  | 1.04x                    |
| Crc32 SSE4 + быстрый DIV | 1.17x                  | 1.08x                    |

В итоге получили ускорение для построения на `17%` и `8%` для построения и поиска соответственно относительно компиляции с флагом -O2.
