...
И теперь таблицу с ускорением

| **Флаги компиляции** | **Построение таблицы** | **Построение + поиск**   |
|----------------------|------------------------|--------------------------|
| Без флагов           | 1.00x                  | 1.00x                    |
| -O2                  | 1.22x                  | 1.13x                    |
| Три оптимизации      | 1.44x                  | 1.22x                    |

В итоге получили ускорение для построения `22%` и `13%` для построения + поиска относительно `-O2`. И на `44%` и на `22%` относительно компиляции без флагов.
